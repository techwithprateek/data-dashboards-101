...
Example prompt:
> "Download the IBM HR Analytics Employee Attrition dataset from Kaggle, compute overall attrition rate, average tenure of leavers vs stayers, and attrition rate by department, overtime status, job role, and tenure bucket. Build a single self-contained HTML dashboard with KPI cards and Chart.js bar charts — one accent color, sorted bars, hover tooltips."
